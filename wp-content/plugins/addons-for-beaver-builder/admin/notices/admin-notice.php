...
abstract class LABB_Notice {
	...
	protected function __construct( $notice_id, $template_file ) {
		...
		$this->option_name      = 'labb_option_dismissed_notices';
	...
	public function dismiss_notice() {
		...
		if ( ! wp_verify_nonce( $_GET['nonce'], 'labb_dismiss_notice' ) ) {
			return;
		}
	...
	protected function get_dismiss_link( $defer = false, $redirect = '' ) {
		...
		$link = admin_url( sprintf(
				...
				'labb_dismiss_notice',
				wp_create_nonce( 'labb_dismiss_notice' ),
				$this->notice_id
			) );
